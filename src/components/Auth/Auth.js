...
class Auth extends Component {
    ...
    register = () => {
        ...
        {username: this.state.username, password: this.state.password, picture: 'https://robohash.org/asdf'})
        .then(res =>{
            this.props.updateUser({
                username: this.state.username, 
                password: this.state.username
        ...
        return(
            <div id='auth-background'>
                <div id='auth-container'>
                    ...
                    <h1>Helo</h1>
                    <div className='input'>
                        Username:<input name='username' value={this.state.username} onChange={e => this.handleInput(e.target)} />
                    </div>
                    <div>
                        Password:<input name='password' value={this.state.password} onChange={e => this.handleInput(e.target)} />
                    </div>
                    <div>
...
const mapStateToProps = reduxState => {
    const username = {username: reduxState.reducer.username}
    return username;
}
